...
//HTTP요청 -> WAS톰캣(서블릿 컨테이너) -> 거대한 하나의 Servlet(SpringMVC) -> 디스패치 서블릿 -> 핸들러 매퍼
//      -> 컨트롤러 -> 매핑 메소드(서블릿) -> 서비스(Service) -> DTO -> DAO(Repository) -> Entity -> 데이터베이스
// HTTP 응답 <- html(json) <- SpringMVC <- 뷰 리졸버(view resolver) <- 동적html파일 <- 디스패치 서블릿
...
public class BoardService {
    ...
    public BoardResponseDto findById(Long boardIdx){
        Board entity = boardRepository.findById(boardIdx).orElseThrow(
    ...
    public Board update(final Long boardIdx, final BoardSaveRequestDto dto){
        ...
        entity.update(dto.getBoardName(), dto.getBoardTitle(), dto.getBoardContent(), dto.getBoardHit());
        //save()함수를 쓰지 않아도 update가 된다 -> 엔티티의 영속성 컨텍스트 속성으로 인해 update가 이루어짐
